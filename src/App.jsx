...
function App() {
  ...
  const [isInitializing, setIsInitializing] = useState(true); // 👈 අලුතෙන් දැම්මේ මේක (App එක ලෝඩ් වෙනකන්)
  ...
  useEffect(() => {
    supabase.auth.getSession().then(({ data: { session } }) => {
      ...
      if (session) {
        ...
        setIsInitializing(false); // Session එකක් නැත්නම් කෙලින්ම Login එකට යනවා
      }
    });
  ...
  const fetchProfile = async (userId) => {
    ...
    try {
      ...
      setIsInitializing(false); // Profile එක ගත්තට පස්සේ Loading නවත්තනවා
    }
  };
  ...
  const handleSaveProfile = async (e) => {
    ...
    try {
      const { error } = await supabase.from('profiles').insert([{ 
        id: session.user.id, full_name: profileFullName, gender: profileGender 
      }]);
  ...
  const c = {
    ...
    textFocus: isFemale ? 'text-pink-500' : 'text-blue-500', 
        ...
        <div className="z-10 w-full max-w-md p-8 bg-[#111827]/80 backdrop-blur-xl border border-gray-800 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)] flex flex-col items-center">
          ...
          <form className="w-full flex flex-col gap-5">
            <div>
              ...
              <input type="email" value={authEmail} onChange={(e) => setAuthEmail(e.target.value)} placeholder="admin@inuka.tech" className="w-full bg-[#0b1120] border border-gray-700 rounded-xl px-4 py-3 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" required/>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-2">Passcode</label>
              <input type="password" value={authPassword} onChange={(e) => setAuthPassword(e.target.value)} placeholder="••••••••" className="w-full bg-[#0b1120] border border-gray-700 rounded-xl px-4 py-3 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" required/>
            </div>
            <div className="flex gap-3 mt-4">
              <button onClick={handleSignUp} disabled={isAuthLoading} className="flex-1 py-3 rounded-xl border border-blue-500/30 text-blue-400 hover:bg-blue-500/10 font-bold disabled:opacity-50">Sign Up</button>
              <button onClick={handleSignIn} disabled={isAuthLoading} className={`flex-[2] py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white shadow-[0_0_20px_rgba(59,130,246,0.3)] font-bold disabled:opacity-50`}>{isAuthLoading ? 'Authenticating...' : 'Access System'}</button>
            </div>
          </form>
        </div>
      </div>
        ...
        <div className="z-10 w-full max-w-md p-8 bg-[#111827]/80 backdrop-blur-xl border border-gray-800 rounded-3xl shadow-[0_0_50px_rgba(0,0,0,0.5)]">
          ...
          <form onSubmit={handleSaveProfile} className="flex flex-col gap-6">
            <div>
              ...
              <input type="text" value={profileFullName} onChange={(e) => setProfileFullName(e.target.value)} placeholder="Inuka Sanjula" className="w-full bg-[#0b1120] border border-gray-700 rounded-xl px-4 py-3 text-white focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" required />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-3">Operator Gender (Controls System Theme)</label>
              <div className="flex gap-3">
                <button type="button" onClick={() => setProfileGender('male')} className={`flex-1 flex items-center gap-3 p-4 rounded-xl border transition-all ${profileGender === 'male' ? 'bg-blue-500/10 border-blue-500 text-blue-400 shadow-[0_0_15px_rgba(59,130,246,0.2)]' : 'bg-[#0b1120] border-gray-800 text-gray-500 hover:border-gray-700'}`}>
                   <span className={`w-5 h-5 rounded-full border-2 ${profileGender === 'male' ? 'border-blue-400 bg-blue-400' : 'border-gray-600'}`}></span>
                   Male <span className="text-xl">🧑‍💻</span>
                </button>
                <button type="button" onClick={() => setProfileGender('female')} className={`flex-1 flex items-center gap-3 p-4 rounded-xl border transition-all ${profileGender === 'female' ? 'bg-pink-500/10 border-pink-500 text-pink-400 shadow-[0_0_15px_rgba(236,72,153,0.2)]' : 'bg-[#0b1120] border-gray-800 text-gray-500 hover:border-gray-700'}`}>
                   <span className={`w-5 h-5 rounded-full border-2 ${profileGender === 'female' ? 'border-pink-400 bg-pink-400' : 'border-gray-600'}`}></span>
                   Female <span className="text-xl">👩‍💻</span>
                </button>
              </div>
            </div>
            <button type="submit" disabled={isAuthLoading} className={`mt-4 w-full py-4 rounded-xl ${profileGender === 'female' ? 'bg-pink-600 hover:bg-pink-500 shadow-[0_0_20px_rgba(236,72,153,0.3)]' : 'bg-blue-600 hover:bg-blue-500 shadow-[0_0_20px_rgba(59,130,246,0.3)]'} text-white font-bold transition-all disabled:opacity-50`}>
  ...
  // 5. Standard Dashboard
  return (
    <div className="flex h-screen bg-[#0b1120] text-white overflow-hidden font-sans relative">
      
      {/* Sidebar */}
      <div className="w-64 bg-[#111827] p-6 flex flex-col justify-between shadow-[10px_0_30px_rgba(0,0,0,0.5)] border-r border-gray-800/50 relative z-10">
        <div>
          <h1 className={`text-2xl font-bold ${c.textPrimary} mb-10 tracking-widest`}>INUKA<span className="text-white">.OS</span></h1>
          <nav className="flex flex-col gap-3">
            {navItems.map((item) => (
              <button key={item} onClick={() => setActiveTab(item)} className={`text-left px-4 py-3 rounded-xl transition-all duration-300 font-medium ${activeTab === item ? `${c.bgActiveTab} ${c.textAccent} ${c.shadowModal} ${c.borderActiveTab} border` : 'text-gray-400 hover:text-white hover:bg-[#1e293b]/50'}`}>
                {item}
              </button>
            ))}
          </nav>
      ...
      <div className="flex-1 p-8 overflow-y-auto relative">
        <div className="flex justify-between items-center mb-10">
          <div>
            ...
            <p className="text-gray-400 mt-1">Logged in as <span className={c.textAccent}>{session.user.email}</span></p>
          </div>
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={studyData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                      ...
                      <Bar dataKey="hours" radius={[6, 6, 0, 0]} barSize={35}>
                        {studyData.map((entry, index) => <Cell key={`cell-${index}`} fill={index === 2 ? c.chartPrimary : c.chartSecondary} className="hover:opacity-80 transition-opacity" />)}
                      </Bar>
                    </BarChart>
                  </ResponsiveContainer>
                ) : (
            ...
            <div className="p-6">
              {formType === 'study' && (
                <div className="flex flex-col gap-4">
                  <div><label className="block text-sm font-medium text-gray-400 mb-1">Subject</label><select value={studyForm.subject} onChange={(e) => setStudyForm({...studyForm, subject: e.target.value})} className={`w-full bg-[#0b1120] border border-gray-700 rounded-xl px-4 py-3 text-white focus:${c.borderPrimary} outline-none`}><option value="SFT">SFT</option><option value="ET">ET</option><option value="ICT">ICT</option></select></div>
